...
gulp.task('welcome', () => {
	console.log(colors.red('Starting template gulpfile! Wizz, whirrrrr, bang, pop!'));
})
...
gulp.task('make-cool-shit', () => {
	setTimeout(() => {
		console.log(' ');
		console.log(' ================================================ '.red);
		console.log('  Welcome to the  Starter template.         '.blue.bold);
		console.log('  You\'re good to go.                              '.blue.bold);
		console.log('  Make cool 💩.                                     '.blue.bold);
		console.log('                                - David Babayev    '.blue.dim);
		console.log(' ================================================ '.red);
		console.log(' ');
	}, 500)
})
		...
		axios.get(packagefileURL, {responseType: 'json'})
			.then(response => {
				if (parsedPackage.version === response.data.version) {
					console.log(`Starter template appears to be up to date.`)
				} else {
					...
					console.warn(`Starter template appears to be out of date.`.red.bold.inverse)				
			...
			.catch(error => {
				console.log(`Couldn't fetch starter mod date`, error);
			});
	});
})
